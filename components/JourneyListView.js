...
export default function JourneyListView({
  ...
  const joinJourneyRequest = async (item) => {
    createRequest(user.email, item.journeyId, userToken)
      .then(() => {
        setPopupText(
          "Requested to join the journey, please wait for the onwer's response."
        );
        togglePopup(true);
      })
      .catch((error) => {
  ...
  return (
    <View style={styles.container}>
      {list.map((item, i) => {
        ...
        const isParticipant =
          item.participantEmails && item.participantEmails.includes(user.email);

        return (
          <ListItem
            style={{ marginHorizontal: 10, marginTop: 10 }}
            containerStyle={
              styles(isHappening, currentJourney, item).containerJourneys
            }
...
const styles = (isHappening, currentJourney, item) =>
  StyleSheet.create({
    container: {
      flex: 1,
      display: "flex",
    ...
    containerJourneys: {
      borderColor:
        isHappening || currentJourney == item ? COLORS.mainColor : "white",
      borderWidth: 2,
      borderRadius: 10,
    },
